              ...
              <div key={uIdx} className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                ...
                <div className="space-y-2">
                  {u.lessons.map((l, lIdx) => (
                    <div key={lIdx} className="flex items-center justify-between bg-white rounded-2xl px-3 py-3 border border-slate-100">
                      <div className="min-w-0 pr-2">
                        <div className="text-[12px] font-black text-slate-700 truncate">{l.name}</div>
                        <div className="text-[10px] font-black text-slate-400 mt-1 flex items-center gap-1">
                          <Clock size={10} />
                          <span>{Math.floor(l.time / 60)} دقيقة • {l.examCount} أسئلة</span>
                        </div>
                      </div>
                      <button 
                        className="btn btn-primary text-xs px-4 py-2 shrink-0" 
                        onClick={() => startExam(l, subject.name, term.name, u.title)}
                      >
                        ابدأ
                      </button>
                    </div>
